...
--custom functions :)
--these are, pretty janky and experimental, you can ask me how they're used if you want but i suggest using everything else
function findNearestTerrainDown(x, y, rate)
	local newy = y
	local sw, sh = Stage.getDimensions()
	while not Stage.collidesPoint(x, newy) do
		newy = newy+rate
		if newy > sh then
			return y
		end
...
function callRandomizer(value, accessor, step) --uses a variable tied to an instance to generate a pseudorandom value
	if value and accessor and accessor:isValid() then
		if not accessor:getData()._number then accessor:getData()._number = 1 
		else accessor:getData()._number = accessor:getData()._number + 1 end
		local newvalue = accessor:getData()._number
		--[[
		if accessor:getData()._number > 12 then accessor:getData()._number = 1 end
		local newvalue = value % accessor:getData()._number
		]]
		local steps = step
		if not step then steps = 5 end
		
		for i = 1, steps do --CHAOS THEORY IN ACTION.
			if newvalue % 2 == 1 then
				newvalue = 3*newvalue + 1
			else
				newvalue = newvalue/2
			end
		end --I CAN DO ANYTHING, HAHAHHAHAHAHAHAHAHAHAHAHAHHAHAHAHAHAHAHHAHAHAHAHAHAHAHHAHAHAHAHAHAHHAHAHAHAAAAAA
...
function counter(accessor) --simply a lifetime counter tied to an instance
	if accessor and accessor:isValid() then
		local accessorData = accessor:getData()
		if not accessorData._timer then accessorData._timer = 0 end
		--[[
		if accessorData._timer % 2 == 1 then accessorData._timer = 3*accessorData._timer + 1 
		else accessorData._timer = accessorData._timer/2 end 
		]]
		accessorData._timer = accessorData._timer + 1
		return accessorData._timer
	end
end
...
end --shamelessly stolen from gmod
